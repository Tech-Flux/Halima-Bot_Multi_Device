...
if (!effect) throw `*Use the command as follows:-*\n💫 *${usedPrefix + command} effect text*\n\nExample\n💫 *${usedPrefix + command} American-flag-3D Halima-Bot*\n\nIncase text is missing\n💫 *${usedPrefix + command} effect text1 | text2*\n\nExample\n💫 *${usedPrefix + command} Wolf-Logo-Galaxy Halima-Bot*\n\n┌─⊷☾ *✨ʟɪsᴛ ᴏғ ʟᴏɢᴏs* ☽︎━━◈\n*┃▢* ${usedPrefix + command} ` + effects.map(v => v.title).join(`\n*┃▢* ${usedPrefix + command} `)
effect = effect.toLowerCase()
if (!effects.find(v => (new RegExp(v.title, 'gi')).test(effect))) throw `*⚠️The Logo ${effect} is not in the list of my ability*`
let text = txt.replace(new RegExp(effect, 'gi'), '').trimStart()
if (text.includes(split)) text = text.split(split)
text = Array.isArray(text) ? text : [text]
let res = await textpro(effect, ...text)
if (typeof res == 'number') throw res == -1 ? `*⚠️The  ${effect} is not in the list of my ability*` : `\n*Use the command as follows:-*\n\n${usedPrefix + command} ${effect} ${new Array(res).fill('*ʏᴏᴜʀ_ɴᴀᴍᴇ').map((v, i) => v + (i ? i + 1 : '')).join('|')}*`
let result = await axios.get(res, {
responseType: 'arraybuffer'
})
await conn.sendFile(m.chat, result.data, 'Error.jpg', `*HERE IS YOUR LOGO✅*\n\n*EFFECT-NAME: ${effect}*\n\n*ᴍᴀᴅᴇ ᴡɪᴛʜ ❤️ ʙʏ ᴀʙᴅᴜʟ*`, m)
}
handler.help = ['logo']
handler.tags = ['']
