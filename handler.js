        ...
        for (let name in global.plugins) {
            ...
            if ((usedPrefix = (match[0] || '')[0])) {
                ...
                try {
                    ...
                    if (m.diamond)
                        m.reply(`You have used *${+m.diamond}* Diamond💎`)
                }
                break
            }
        }
        ...
        case 'remove':
            if (chat.welcome) {
                ...
                for (let user of participants) {
                    let pp = 'https://i.imgur.com/VHsqCpl.jpg'
                    let ppgp = 'https://i.imgur.com/IPYoJor.jpg'
                    try {
                        ...
                        text = (action === 'add' ? (chat.sWelcome || this.welcome || conn.welcome || 'Welcome, @user').replace('@group', await this.getName(id)).replace('@desc', groupMetadata.desc?.toString() || 'Unknown') :
                            ...
                            let wel = API('fgmods', '/api/welcome', {
                                ...
                                profile: pp,
                                background: 'https://i.imgur.com/EJgRBnS.jpg'
                            }, 'apikey')
            ...
            break
        case 'promote':
            text = (chat.sPromote || this.spromote || conn.spromote || '@user You are now an administrator')
        case 'demote':
            let pp = await this.profilePictureUrl(participants[0], 'image').catch(_ => 'https://i.imgur.com/ER84fYd.jpg') 
            if (!text)
                text = (chat.sDemote || this.sdemote || conn.sdemote || '@user You are no longer an administrator')
...
global.dfail = (type, m, conn) => {
    let msg = {
        rowner: '👑 This command can only be used by *Abdul*',
        owner: '👑 *_This command can only be used by Abdul_*',
        mods: '🔰  This feature is for *For Bot moderators only*',
        premium: '💠 This command is for *Premium* members only\n\nType *.premium* for more info',
        group: '⚙️ *Sorry!* *This command can only be used in groups!*',
